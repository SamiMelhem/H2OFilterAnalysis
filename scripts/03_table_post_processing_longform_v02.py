...
sorting = pd.read_csv(os.path.join(input_dir, 'class_constituent_ordering.csv'), index_col='Constituent')
...
def decision_function(dfs):
    ...
    for k in dfs.keys():
        dfs[k]['Final Recommendation'] = np.nan
    for k in dfs.keys():
        for row in range(len(dfs[k])):
            if (dfs[k]['Detection Rate'].iloc[row] > 0) & (dfs[k]['Detection Rate'].iloc[row] <= 0.3):
                # print(dfs[k]['Regulatory Limit'])
                # print('low detection rate')
                if dfs[k]['Max_adj'].iloc[row] < 0.5*dfs[k]['Regulatory Limit'].iloc[row]:
                    dfs[k]['Final Recommendation'].iloc[row] = 'Low det. rate, Max <50% Reg. Limit'
                elif dfs[k]['Max_adj'].iloc[row] >= 0.5*dfs[k]['Regulatory Limit'].iloc[row]:
                    dfs[k]['Final Recommendation'] = 'Low det. rate, Max >50% Reg. Limit'
                    print('greater than')
                else:
                    pass
                # elif dfs[k]['Regulatory Limit'].iloc[row].isna():
                #     dfs[k]['Final Recommendation'].iloc[row] = 'Low det. rate, no Reg. Limit'

    df = dfs['ABP WDR'] #.loc[dfs['ABP WDR']['Category'] == 'Radioactivity']

    print(df['Final Recommendation'].unique())

    ## if detection rate < 30%
            ## if max value > 50% reg. limit and trend increasing
                ## if year > 5 yrs -- keep
                ## if year < 5 yrs -- reduce
                ## else reduce (no trend/trend decreasing or <50% limit/no limit)
    ## if detection rate > 30%
            ## if max value >= 50% reg. limit -- keep (regardless of trend)
            ## if max value < 50% reg. limit -- reduce
            ## if no limit & increasing -- keep
            ## if no limit & decreasing/no trend -- reduce
